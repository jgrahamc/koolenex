...
export function DeviceProductTab({ dev, C }) {
  const searchQuery = [dev.manufacturer, dev.order_number || dev.model].filter(Boolean).join(' ') + ' manual';
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 11, color: C.accent, fontWeight: 600, letterSpacing: '0.08em', marginBottom: 12 }}>PRODUCT INFO</div>
        {[
          ...
          <div key={label} style={{ display: 'flex', fontSize: 11, marginBottom: 5 }}>
        ...
        <div style={{ marginTop: 12 }}>
          <a href={searchUrl} target="_blank" rel="noopener noreferrer"
            style={{ fontSize: 11, color: C.accent, textDecoration: 'none', padding: '6px 12px', border: `1px solid ${C.accent}40`, borderRadius: 4, display: 'inline-block' }}>
