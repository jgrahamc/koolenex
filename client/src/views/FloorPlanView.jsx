...
const COLMAP = { actuator: '#4fc3f7', sensor: '#aed581', router: '#ffb74d', generic: '#999' };

import { AddDeviceModal } from '../AddDeviceModal.jsx';

export function FloorPlanView({ data, activeProjectId, onUpdateDevice, jumpTo, onAddDevice }) {
  const C = useC();
  ...
  const { floors, floorDevices } = useMemo(() => {
    ...
    for (const floor of floors) {
      ...
      const walk = (node) => { spaceIds.add(node.id); node.children.forEach(walk); };
      walk(floor);
      floorDevices[floor.id] = devices.filter(d => d.space_id && spaceIds.has(d.space_id));
    }
  ...
  useEffect(() => {
    if (floors.length > 0 && (!activeFloor || !floors.find(f => f.id === activeFloor))) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Tab bar */}
      <div style={{ display: 'flex', borderBottom: `1px solid ${C.border}`, flexShrink: 0, overflow: 'auto' }}>
        {floors.map(f => (
          <div key={f.id} onClick={() => setActiveFloor(f.id)}
            style={{
              padding: '8px 16px', fontSize: 11, cursor: 'pointer',
              borderBottom: f.id === activeFloor ? `2px solid ${C.accent}` : '2px solid transparent',
...
function FloorPlanCanvas({ floor, devices, spaces, projectId, onUpdateDevice, onAddDevice, data, pin, C }) {
  ...
  const appZoom = parseFloat(document.getElementById('root')?.firstChild?.style?.zoom) || 1;
  ...
  const startDrag = (e, deviceId) => {
    ...
    if (dev && dev.floor_x >= 0 && clickPos) {
      dragOffsetRef.current = { dx: dev.floor_x - clickPos.x, dy: dev.floor_y - clickPos.y };
    } else {
  ...
  const onDragOverlayMove = useCallback((e) => {
  ...
  const onDragOverlayUp = useCallback((e) => {
    const pos = getFrac(e.clientX, e.clientY);
    const off = dragOffsetRef.current || { dx: 0, dy: 0 };
  ...
  if (!imgUrl) {
    return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
        <div style={{ fontSize: 11, color: C.muted }}>No floor plan image for {floor.name}</div>
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', position: 'relative', background: C.bg, zoom: 1 / appZoom }}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderRadius: 4 * appZoom, padding: `${2*appZoom}px ${6*appZoom}px`,
                  fontSize: 9 * appZoom, color: C.text, whiteSpace: 'nowrap',
                  userSelect: 'none',
                }}>
                <DeviceTypeIcon type={d.device_type} size={10 * appZoom} style={{ color: COLMAP[d.device_type] || C.muted }} />
                <span style={{ maxWidth: 80 * appZoom, overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.name}</span>
                <span
                  onMouseDown={(e) => { e.stopPropagation(); e.preventDefault(); onUpdateDevice(d.id, { floor_x: -1, floor_y: -1 }); }}
                  title="Remove from floor plan"
                  style={{ marginLeft: 2, cursor: 'pointer', color: C.dim, fontSize: 8 * appZoom, lineHeight: 1 }}>✕</span>
              </div>
            );
          })}
        </div>
        {/* Controls — inside the canvas scroll area, zoom reset so they render at normal size */}
        <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', gap: 6, zIndex: 300, zoom: appZoom }}>
          <input ref={fileRef} type="file" accept="image/*" onChange={handleUpload} style={{ display: 'none' }} />
          {onAddDevice && <Btn onClick={() => setShowAdd(true)} style={{ fontSize: 9, padding: '3px 8px' }} color={C.green}>+ Add Device</Btn>}
          <Btn onClick={() => fileRef.current?.click()} style={{ fontSize: 9, padding: '3px 8px' }}>Replace image</Btn>
          <Btn onClick={handleDelete} style={{ fontSize: 9, padding: '3px 8px' }}>Remove</Btn>
        </div>
      </div>
      {showAdd && onAddDevice && <AddDeviceModal data={data} defaults={{ space_id: floor.id }} onAdd={onAddDevice} onClose={() => setShowAdd(false)} />}

      {/* Unplaced devices sidebar */}
      {unplaced.length > 0 && (
        <div style={{ width: 180, borderLeft: `1px solid ${C.border}`, overflow: 'auto', flexShrink: 0, padding: 8 }}>
          <div style={{ fontSize: 9, color: C.dim, letterSpacing: '0.08em', marginBottom: 8 }}>
            UNPLACED ({unplaced.length})
          </div>
          {Object.entries(devicesByRoom).map(([room, roomDevs]) => {
            const unplacedInRoom = roomDevs.filter(d => d.floor_x < 0 || d.floor_y < 0);
            if (!unplacedInRoom.length) return null;
            return (
              <div key={room} style={{ marginBottom: 10 }}>
                <div style={{ fontSize: 9, color: C.muted, marginBottom: 4 }}>{room}</div>
                {unplacedInRoom.map(d => (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 9, color: C.text, cursor: 'grab',
                      userSelect: 'none',
                    }}>
                    <DeviceTypeIcon type={d.device_type} size={10} style={{ color: COLMAP[d.device_type] || C.muted }} />
