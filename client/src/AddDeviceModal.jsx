...
function nextDeviceNum(devices, area, line) {
  const used = new Set(
    devices.filter(d => d.area === area && d.line === line)
...
export function AddDeviceModal({ data, defaults = {}, onAdd, onClose }) {
    ...
    for (const d of devices) {
      ...
      if (!map[d.manufacturer][key]) {
        map[d.manufacturer][key] = {
          ...
          order_number: d.order_number || '', medium: d.medium || 'TP',
          description: d.description || '', product_ref: d.product_ref || '',
        };
      }
    }
    return Object.entries(map).sort((a, b) => a[0].localeCompare(b[0])).map(([mfr, models]) => ({
      name: mfr,
      models: Object.entries(models).sort((a, b) => a[0].localeCompare(b[0])).map(([name, info]) => ({ name, ...info })),
    }));
  }, [devices]);
  ...
  const [devNum, setDevNum] = useState(() => {
    const a = defaults.area ?? 1, l = defaults.line ?? 1;
  ...
  const handleAreaChange = (a) => { setArea(a); setDevNum(nextDeviceNum(devices, a, line) ?? 1); };
  const handleLineChange = (l) => { setLine(l); setDevNum(nextDeviceNum(devices, area, l) ?? 1); };
  ...
  const searchResults = useMemo(() => {
    ...
    return filteredTree.map(mfr => ({
      ...mfr,
      models: mfr.models.filter(m =>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}
      onClick={onClose}>
      <div style={{ background: C.surface, border: `1px solid ${C.border2}`, borderRadius: 8, padding: 20, width: 480, maxHeight: '80vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}
        onClick={e => e.stopPropagation()}>
        <div style={{ fontSize: 12, fontWeight: 600, color: C.text, marginBottom: 14 }}>Add Device</div>

        {/* Device type picker — hidden when type is pre-selected */}
        {typeFixed ? (
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 9, color: C.dim, letterSpacing: '0.08em', marginBottom: 4 }}>DEVICE TYPE</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', background: C.bg, border: `1px solid ${C.border}`, borderRadius: 4 }}>
              {modelInfo && <DeviceTypeIcon type={modelInfo.device_type} size={11} style={{ color: C.muted }} />}
              <span style={{ fontSize: 10, color: C.text }}>{selectedMfr}{selectedModel ? ` — ${selectedModel}` : ''}</span>
              {modelInfo?.order_number && <span style={{ fontSize: 9, color: C.dim, marginLeft: 'auto' }}>{modelInfo.order_number}</span>}
            </div>
          </div>
        ) : (<>
        <div style={{ fontSize: 9, color: C.dim, letterSpacing: '0.08em', marginBottom: 4 }}>DEVICE TYPE</div>
        ...
        <div style={{ maxHeight: 150, overflow: 'auto', border: `1px solid ${C.border}`, borderRadius: 4, marginBottom: 14 }}>
          {searchResults.length === 0 && (
            <div style={{ padding: 12, fontSize: 10, color: C.dim, textAlign: 'center' }}>No matching device types</div>
          )}
          {searchResults.map(mfr => (
            <div key={mfr.name}>
              <div style={{ fontSize: 9, color: C.dim, padding: '4px 8px', background: C.bg, borderBottom: `1px solid ${C.border}` }}>{mfr.name}</div>
              {mfr.models.map(m => {
                const isSel = selectedMfr === mfr.name && selectedModel === m.name;
                return (
                  <div key={m.name} onClick={() => handleModelSelect(mfr, m)}
                    style={{
                    ...
                    className="rh">
                    <DeviceTypeIcon type={m.device_type} size={11} style={{ color: C.muted }} />
                    <span style={{ fontSize: 10, color: isSel ? C.accent : C.text }}>{m.name}</span>
                    {m.order_number && <span style={{ fontSize: 9, color: C.dim, marginLeft: 'auto' }}>{m.order_number}</span>}
                  </div>
                );
              })}
            </div>
          ...
          {/* Generic option */}
          <div onClick={() => { setSelectedMfr(''); setSelectedModel(''); setName(name || 'New Device'); }}
            style={{
              padding: '4px 8px', cursor: 'pointer', fontSize: 10, color: C.muted,
              background: !selectedMfr && !selectedModel ? `${C.accent}20` : 'transparent',
            }}
            className="rh">
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <Btn onClick={handleSubmit} color={C.accent} disabled={addressExists}>Add Device</Btn>
        </div>
      </div>
    </div>
  );
