...
export let _i18nT = (refId) => null;
export let _i18nLang = 'en-US';
export function setI18nT(fn) { _i18nT = fn; }
export function setI18nLang(lang) { _i18nLang = lang; }
...
export function localizedModel(device) {
  ...
  if (_i18nLang && device.model_translations) {
    try {
      const tr = typeof device.model_translations === 'string'
...
export function setSpaceUsages(data) { SPACE_USAGES = data; }
export const spaceUsageMap = () => Object.fromEntries(SPACE_USAGES.map(su => [su.id, su.text]));

export function setDptInfo(data) { DPT_INFO = data; }
...
export function dptInfo(dpt) {
  ...
  return DPT_INFO[d] || DPT_INFO[d.split('.')[0] + '.001'] || { name: d, text: '', unit: '' };
}

export function dptUnit(dpt) { return dptInfo(dpt).unit; }
...
export function dptTitle(dpt) {
  ...
  return translated ? `${translated} — ${name}` : text ? `${text} — ${name}` : unit ? `${name} (${unit.trim()})` : name;
}
